...
@section('content')
                    ...
                    <tbody>
                        @foreach ($todayBookings as $booking)
                            <tr>
                                ...
                                <td>{{ $booking->start_time->format('H:i') }} - {{ $booking->end_time->format('H:i') }}
                                </td>
                                <td>
                                    ...
                                    <form action="{{ route('admin.bookings.updateStatus', $booking) }}" method="POST">
                                        ...
                                        <select name="status" class="form-select form-select-sm"
                                            ...
                                            <option value="complete"
                                                {{ $booking->status == 'complete' ? 'selected' : '' }}>Complete</option>
    ...
    <div class="card glass-card shadow-sm mb-4">
        <div class="card-header border-0 bg-transparent">
            <h5><i class="bi bi-calendar-event me-2"></i> ปฏิทินการจอง</h5>
        </div>
        <div class="card-body">
            <div id="calendar"></div>
        </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // ดึงข้อมูลการจองจาก Controller
            const calendarBookings = @json($calendarBookings ?? []);

            // Map ข้อมูลเป็น Event ของ FullCalendar
            const events = calendarBookings.map(b => ({
                title: b.room_name + ' - ' + (b.user_name || 'N/A'),
                start: b.start_time,
                end: b.end_time,
                color: '#e74c3c', // สีแดงแสดงว่าถูกจองแล้ว
                extendedProps: {
                    status: b.status,
                    user: b.user_name,
                    room: b.room_name
            ...
            <p><strong>User:</strong> ${info.event.extendedProps.user}</p>
        ...
        `,
                        ...
                        if (result.isConfirmed) {
                            // เปลี่ยนลิงก์ตาม Route แก้ไข Booking ของคุณ
                            window.location.href = `/admin/bookings/${info.event.id}/edit`;
                        }
                    });
                }
            });
